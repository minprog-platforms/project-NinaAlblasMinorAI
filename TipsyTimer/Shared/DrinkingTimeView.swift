...
struct DrinkingTimeView: View {
    ...
    var body: some View {
        VStack {
            ...
            ZStack {
                Circle()
                    .stroke(Color("Nina-sky"), style: StrokeStyle(lineWidth: getFrameWidth(45), lineCap: .round, lineJoin: .round))
                    .frame(width: getFrameWidth(300), height: getFrameWidth(300))
                    .rotationEffect(.init(degrees: -90))
                    .shadow(color: Color("Nina-dark"), radius: 1)
                Circle()
                    .trim(from: 0, to: progress)
                    .stroke(Color("Nina-pinkpurple"), style: StrokeStyle(lineWidth: getFrameWidth(45), lineCap: .round, lineJoin: .round))
                    .frame(width: getFrameWidth(300), height: getFrameWidth(300))
                    .rotationEffect(.init(degrees: -90))
                    .shadow(color: Color("Nina-dark"), radius: 5)
                Circle()
                    .frame(width: getFrameWidth(45), height: getFrameWidth(45))
                    .foregroundColor(Color("Tipsy-white"))
                    .offset(x: getOffsetX(150))
